...
exports.sourceNodes = async ({ actions, reporter }, { fileId, nodeIds, projectId, scale, format, accessToken }) => {
...
See docs here – https://github.com/fabe/gatsby-source-figma
  ...
  if (fileId) {
    ...
    const projectFiles = project.files.map(file => Figma.fetchFile(file.key, accessToken));

    files = await Promise.all(projectFiles);
  }

  if (fileId && nodeIds) {
    const imagesData = nodeIds.map(nodeId => Figma.fetchNode(fileId, nodeId, scale, format, accessToken));
